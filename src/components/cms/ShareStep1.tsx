...
import { loadContacts, getContactGroupCounts } from '../../utils/contact-storage';
import { loadCustomGroups, CustomGroup, getColorClasses } from '../../utils/custom-groups';
...
export function ShareStep1({ onAddContact, onSelectContact, onShareGroup }: ShareStep1Props) {
  ...
  const groupCounts = getContactGroupCounts();
  const scrollContainerRef = useRef<HTMLDivElement>(null);
  const [canScrollLeft, setCanScrollLeft] = useState(false);
  const [canScrollRight, setCanScrollRight] = useState(true);

  // Refresh groups when component mounts or becomes visible
  useEffect(() => {
            ...
            {filteredContacts.length === 0 ? (
              ...
              filteredContacts.map((contact) => (
                <button
                  ...
                  className="content-stretch flex gap-[16px] items-center relative shrink-0 w-full hover:bg-muted transition-colors rounded-lg p-2 -m-2"
                >
